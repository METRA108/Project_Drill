...
const green = preload("res://sprites/green_eye.png")
...
func _process(_delta):
	if Input.is_action_just_pressed("ui_text_completion_accept"):
		get_node("/root/Node2D/CharacterBody/Eye").set_texture(idle)
		
	if Input.is_action_just_released("ui_text_completion_accept"):
		get_node("/root/Node2D/CharacterBody/Eye").set_texture(green)
